...
PANEL_USER="carsten-panel"
PANEL_HOME="/opt/carsten-webpanel"
DATA_DIR="/var/lib/carsten-webpanel"
SITE_ROOT="/srv/carsten-sites"
PANEL_PORT="9090"
...
install -o root -g root -m 0755 /dev/null /usr/local/sbin/carsten-panel-apply
cat > /usr/local/sbin/carsten-panel-apply <<'PYHELPER'
#!/usr/bin/env python3
import json, os, re, subprocess, sys, tempfile
from pathlib import Path

DATA = Path("/var/lib/carsten-webpanel/sites.json")
OUT = Path("/etc/nginx/carsten-sites")
ROOT = Path("/srv/carsten-sites")
    ...
    if ufw.exists():
        ...
        if "Status: active" in status.stdout:
            for port in sorted({s["port"] for s in sites if s.get("active", True)}):
                ...
                if is_public:
                    subprocess.run([str(ufw), "allow", f"{port}/tcp", "comment", "Carsten public site"], check=True, stdout=subprocess.DEVNULL)
                else:
...
if __name__ == "__main__":
    main()
PYHELPER
chmod 0755 /usr/local/sbin/carsten-panel-apply
chown root:root /usr/local/sbin/carsten-panel-apply
...
app = Flask(__name__)
DATA = Path("/var/lib/carsten-webpanel")
...
def apply_config():
    result = subprocess.run(["sudo", "/usr/local/sbin/carsten-panel-apply"], text=True, capture_output=True)
...
BASE = r'''<!doctype html><html lang="da"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{{ title }} · Carsten</title><style>''' + STYLE + r'''</style></head><body>
{% if session.get('logged_in') %}<div class="shell"><aside class="side"><div class="brand"><span class="logo">C</span> Carsten Panel</div><nav class="nav"><a class="on" href="{{ url_for('dashboard') }}">● Websites</a><a href="{{ url_for('new_site') }}">＋ Tilføj website</a><a href="{{ url_for('system') }}">◫ Serverinfo</a><a href="{{ url_for('logout') }}">↪ Log ud</a></nav><div class="side-foot">Nginx · Ubuntu · gratis</div></aside><main class="main">{% else %}<div class="login-wrap"><main class="card login">{% endif %}
...
install -d -o root -g root -m 0755 /etc/nginx/carsten-sites
cat > /etc/nginx/conf.d/carsten-sites.conf <<'NGINXINCLUDE'
include /etc/nginx/carsten-sites/*.conf;
NGINXINCLUDE
...
NGINXPANEL
ln -sfn /etc/nginx/sites-available/carsten-panel /etc/nginx/sites-enabled/carsten-panel

echo "[5/7] Opretter automatisk opstart..."
cat > /etc/systemd/system/carsten-webpanel.service <<EOF
[Unit]
Description=Carsten Webpanel
...
PrivateTmp=true
ProtectSystem=full
ReadWritePaths=${DATA_DIR} ${SITE_ROOT} /etc/nginx/carsten-sites -/etc/ufw -/var/lib/ufw
ProtectHome=true
...
cat > /etc/sudoers.d/carsten-webpanel <<EOF
${PANEL_USER} ALL=(root) NOPASSWD: /usr/local/sbin/carsten-panel-apply
EOF
chmod 0440 /etc/sudoers.d/carsten-webpanel

echo "[6/7] Tester opsætningen..."
/usr/sbin/visudo -cf /etc/sudoers.d/carsten-webpanel
